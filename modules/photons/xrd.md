...
Here $d_{hkl}$ is the spacing of the planes, $\theta$ the angle between the beam and those planes, $n$ the order of the reflection, and $\lambda$ the X-ray wavelength. Each set of lattice planes, labeled by Miller indices $(hkl)$, has its own spacing $d_{hkl}$ (for a cubic crystal of lattice parameter $a$, $d_{hkl} = a/\sqrt{h^2+k^2+l^2}$) and therefore its own Bragg angle. A measured pattern of peak positions is thus a fingerprint of the lattice, and comparing measured positions and intensities against the powder diffraction database is how phases are identified.
