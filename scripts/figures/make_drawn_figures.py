...
for ax, mode in zip(axes, ["sym", "gixrd"]):
    ...
    ax.text(-1.30, -0.06, "film", fontsize=12)
    ax.text(-1.30, -0.24, "substrate", fontsize=12)
    if mode == "sym":
        th = np.radians(35)
...
ax.text(0.74, 0.57, "thin sample", fontsize=12)
...
for name, a0, a1, c, rl in det:
    ...
    ax.text(1.18*np.sin(np.radians((np.degrees(np.arctan(a1*scale)) +
            np.degrees(np.arctan(a0*scale)))/2)), 0.60 - 1.18, "", fontsize=10.5)
...
for name, a0, a1, c, rl in det:
    ...
    ax.text(1.36, y0 - 0.005, "", fontsize=10.5)
labels_y = {-0.78: None}
ly = y0 - 0.14
for i, (name, a0, a1, c, rl) in enumerate(det):
    ax.text(-1.38, ly - i*0.115, name, fontsize=11.5, color=c)
ax.text(0.07, 0.80, "converged probe", fontsize=12, color=st.BLUE)
