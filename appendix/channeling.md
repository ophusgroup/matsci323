...
Channeling extends [RBS](../modules/ions/rbs.md) from composition into crystallography. It appears here as reference material: a beautiful technique, historically central to semiconductor science, now practiced at the same few accelerator laboratories as RBS itself.

When an MeV ion beam is aligned with a low-index axis of a single crystal, the atomic rows act as gentle guides: ions entering between rows are steered by a series of correlated small-angle collisions and never approach a lattice atom closely enough to backscatter. The backscattering yield collapses, to a few percent of the random-orientation value for a good crystal, as long as the beam stays within a critical angle of the axis,
...
Channeling requires a goniometer, patience, and a single crystal, and its niches have largely been inherited by X-ray diffraction (strain, quality) and STEM (defects, interfaces). It earns its appendix both for the physics, which reappears as electron channeling in EBSD patterns and STEM imaging of aligned crystals, and because aligned-beam artifacts can surprise anyone doing RBS on epitaxial samples: a substrate that channels accidentally reads as anomalously transparent.
